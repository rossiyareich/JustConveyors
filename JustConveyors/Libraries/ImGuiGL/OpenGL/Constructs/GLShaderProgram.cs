...
namespace JustConveyors.Libraries.ImGuiGL.OpenGL.Constructs;
...
public sealed class GLShaderProgram : IDisposable
{
    ...
    public GLShaderProgram(string vertexShaderSource, string fragmentShaderSource)
        : this(new GLShader(vertexShaderSource, ShaderType.VertexShader),
            new GLShader(fragmentShaderSource, ShaderType.FragmentShader)) =>
    ...
    private void GetParams()
    {
        ...
        for (uint i = 0; i < resources[0]; i++)
        {
            ...
            {
                GLShaderProgramParam param = new(TypeFromAttributeType(type[0]),
                    ParamType.Attribute, sb.ToString());
        ...
        for (uint i = 0; i < resources[0]; i++)
        {
            ...
            if (!shaderParams.ContainsKey(sb.ToString()))
            {
                GLShaderProgramParam param = new(TypeFromUniformType(type[0]), ParamType.Uniform, sb.ToString());
                shaderParams.Add(param.Name, param);
                param.GetLocation(this);
            }
        }
