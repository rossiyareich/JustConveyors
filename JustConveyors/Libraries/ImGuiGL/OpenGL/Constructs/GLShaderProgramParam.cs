...
public sealed class GLShaderProgramParam
{
    ...
    public GLShaderProgramParam(Type type, ParamType paramType, string name, uint program, int location) : this(type,
        paramType, name)
    {
        ProgramId = Program;
        Location = location;
    ...
    public void GetLocation(GLShaderProgram Program)
    {
        ...
        if (ProgramId == 0)
        {
            ...
            Location = ParamType == ParamType.Uniform
    ...
    private void EnsureType<T>() =>
        Debug.Assert(Type == typeof(T), $"SetValue({Type}) was called with a {typeof(T).Name}");
}
